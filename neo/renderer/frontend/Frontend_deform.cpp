...
=================
*/
void crFrontend::FinishDeform( drawSurf_t *drawSurf, srfTriangles_t *newTri, idDrawVert *ac ) 
{
	if ( !newTri ) 
		return;
...
=====================
*/
void crFrontend::TubeDeform( drawSurf_t *surf ) 
{
	int		i, j;
	int		indexes;
	const srfTriangles_t *tri;
...
void crFrontend::FlareDeform( drawSurf_t *surf ) 
{
	const srfTriangles_t *tri;
	srfTriangles_t		*newTri;
	idPlane	plane;
...
=====================
*/
void crFrontend::ExpandDeform( drawSurf_t *surf ) 
{
	int		i;
	const srfTriangles_t	*tri;
	srfTriangles_t	*newTri;
...
=====================
*/
void crFrontend::MoveDeform( drawSurf_t *surf ) 
{
	int		i;
	const srfTriangles_t	*tri;
	srfTriangles_t	*newTri;
...
=====================
*/
void crFrontend::TurbulentDeform( drawSurf_t *surf )
{
	int		i;
	const srfTriangles_t	*tri;
	srfTriangles_t	*newTri;
...
=====================
*/
void crFrontend::EyeballDeform( drawSurf_t *surf ) 
{
	int		i = 0, j = 0, k = 0;
...
=====================
*/
void crFrontend::ParticleDeform( drawSurf_t *surf, bool useArea ) 
{
...
#if 0
	if ( renderEntity->shaderParms[SHADERPARM_PARTICLE_STOPTIME] && viewDef->renderView.time*0.001 >= renderEntity->shaderParms[SHADERPARM_PARTICLE_STOPTIME] ) 
		return NULL; // the entire system has faded out
#endif
	...
	for ( int currentTri = 0; currentTri < ( ( useArea ) ? 1 : numSourceTris ); currentTri++ ) 
	{
		for ( int stageNum = 0 ; stageNum < particleSystem->stages.Num() ; stageNum++ ) 
		{
			...
			for ( int index = 0 ; index < totalParticles ; index++ ) 
			{
				...
				if ( particleCycle < 0 )
					continue; // before the particleSystem spawned
				
				if ( stage->cycles && particleCycle >= stage->cycles ) 
					continue; // cycled systems will only run cycle times
				...
				if ( renderEntity->shaderParms[SHADERPARM_PARTICLE_STOPTIME] && g.renderView->time - inCycleTime >= renderEntity->shaderParms[SHADERPARM_PARTICLE_STOPTIME]*1000 ) 
					continue; // don't fire any more particles

				// supress particles before or after the age clamp
				g.frac = (float)inCycleTime / ( stage->particleLife * 1000 );
				if ( g.frac < 0 ) 
					continue; // yet to be spawned
				
				if ( g.frac > 1.0 )
					continue; // this particle is in the deadTime band
			...
			if ( tri->numVerts > 0 ) 
			{
				...
				{
					// add the drawsurf
					AddDrawSurf( tri, surf->space, renderEntity, stage->material, surf->scissorRect );
				}
			}
		}
	}
...
=================
*/
void crFrontend::DeformDrawSurf( drawSurf_t *drawSurf ) 
{
	...
	case DFRM_NONE:
		return;
	case DFRM_SPRITE:
		AutospriteDeform( drawSurf );
		break;
	case DFRM_TUBE:
		TubeDeform( drawSurf );
		break;
	case DFRM_FLARE:
		FlareDeform( drawSurf );
		break;
	case DFRM_EXPAND:
		ExpandDeform( drawSurf );
		break;
	case DFRM_MOVE:
		MoveDeform( drawSurf );
		break;
	case DFRM_TURB:
		TurbulentDeform( drawSurf );
		break;
	case DFRM_EYEBALL:
		EyeballDeform( drawSurf );
		break;
	case DFRM_PARTICLE:
		ParticleDeform( drawSurf, true );
		break;
	case DFRM_PARTICLE2:
		ParticleDeform( drawSurf, false );
		break;
	}
}
