...
static void R_ProjectPointsToFarPlane( const idRenderEntityLocal *ent, const idRenderLightLocal *light,
	...
	lv = ent->modelMatrix.GlobalPointToLocal( light->globalLightOrigin );
...
=================
*/
srfTriangles_t*  crFrontend::CreateShadowVolume( const idRenderEntityLocal *ent, const srfTriangles_t *tri, const idRenderLightLocal *light, shadowGen_t optimize, srfCullInfo_t &cullInfo ) 
{
	int		i, j;
	idVec3	lightOrigin;
	...
	tr.frontend->GetPerformanceCounters().c_createShadowVolumes++;
	...
	if ( optimize == SG_DYNAMIC && r_useTurboShadow.GetBool() )
		R_CreateVertexProgramTurboShadowVolume( ent, tri, light, cullInfo );
	...
	//
	lightOrigin = ent->modelMatrix.GlobalPointToLocal( light->globalLightOrigin );
	...
	for ( int frustumNum = 0 ; frustumNum < light->numShadowFrustums ; frustumNum++ ) 
	{
		...
		{
			frustum[j] = ent->modelMatrix.GlobalPlaneToLocal( frust->planes[j] );
