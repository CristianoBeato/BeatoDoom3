...
public:
    ...
    virtual bool        Create( bufferUsage_t usage, const size_t size, const bool write = true, const bool read = false ) = 0;
    
    /// @brief destroy object and release memory
    /// @param  
    virtual void        Destroy( void ) = 0;

    /// @brief Flush the content of the driver/stage to the GPU
    /// @param offset buffer content offset 
    /// @param size  buffer content size 
    virtual void        Flush( const uintptr_t offset,  const size_t size ) const = 0;
...
public:
    ...
    virtual void    Begin( void ) = 0;
    virtual void    End( void ) = 0;
    virtual void    CopyBufferContent( const uintptr_t srcOffset, const uintptr_t dstOffset, const size_t size, crBuffer* src, crBuffer* dst ) = 0;
