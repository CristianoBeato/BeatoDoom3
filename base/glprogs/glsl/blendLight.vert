...
void main(void)
{
  ...
  vec4 vertexPosition = vec4( attrb_position, 1.0);

  // vertex raster output
  gl_Position = mvp * vertexPosition;

  //FIXME(johl): use real 4x4 texture matrices everywhere (instead of abusing partial bump, specular and diffuse matrix)!
  result.texcoord0.x = dot( vertexPosition, vertUnifom[gl_DrawID].rpTextureMatrixS );
  result.texcoord0.y = dot( vertexPosition, vertUnifom[gl_DrawID].rpTextureMatrixT );
